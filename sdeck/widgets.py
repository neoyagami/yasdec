...
class KeyInspector(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None, action_only: bool = False) -> None:
        ...
        self.spectrum_gain.valueChanged.connect(self._store)
        self.spectrum_fall.valueChanged.connect(self._store)
        self.spectrum_preview.toggled.connect(self._store)
        self.spectrum_auto_scale.toggled.connect(self._store)
        self.spectrum_auto_scale.toggled.connect(lambda checked: self.spectrum_gain.setEnabled(not checked))
        self.spectrum_auto_stop.toggled.connect(self._visual_option_changed)
        self.spectrum_silence_seconds.valueChanged.connect(self._store)
    ...
    def _spectrum_page(self) -> QWidget:
        ...
        self.spectrum_gain = QSpinBox()
        self.spectrum_gain.setRange(-12, 30)
        self.spectrum_gain.setSuffix(" dB")
        self.spectrum_gain.setToolTip(tr("Fixed visual gain; does not change audio volume or adapt to the music"))
        self.spectrum_fall = QSpinBox()
        self.spectrum_fall.setRange(0, 3000)
        self.spectrum_fall.setSingleStep(100)
        self.spectrum_fall.setValue(800)
        self.spectrum_fall.setSuffix(" ms")
        self.spectrum_fall.setToolTip(tr("Time to fall from full height to zero; 0 disables smoothing"))
        self.spectrum_auto_scale.setToolTip(tr("Scales each frame to use the available height instead of fixed dBFS levels"))
        self.spectrum_auto_stop = QCheckBox(tr("Auto-stop when silent"))
        self.spectrum_silence_seconds = QSpinBox()
        self.spectrum_silence_seconds.setRange(5, 3600)
        self.spectrum_silence_seconds.setValue(30)
        self.spectrum_silence_seconds.setSuffix(tr(" seconds"))
        self.spectrum_silence_controls = QWidget()
        spectrum_silence_layout = QHBoxLayout(self.spectrum_silence_controls)
        spectrum_silence_layout.setContentsMargins(0, 0, 0, 0)
        spectrum_silence_layout.addWidget(self.spectrum_auto_stop)
        spectrum_silence_layout.addWidget(self.spectrum_silence_seconds)
        ...
        self.spectrum_form.addRow(tr("Scale"), self.spectrum_auto_scale)
        self.spectrum_form.addRow(tr("Visual gain"), self.spectrum_gain)
        self.spectrum_form.addRow(tr("Bar fall time"), self.spectrum_fall)
        self.spectrum_form.addRow(tr("Inactive view"), self.spectrum_preview)
        self.spectrum_form.addRow(tr("Silence"), self.spectrum_silence_controls)
        return page
    ...
    def _vu_page(self) -> QWidget:
        ...
        self.vu_silence_seconds = QSpinBox()
        self.vu_silence_seconds.setRange(5, 3600)
        self.vu_silence_seconds.setValue(30)
        self.vu_silence_seconds.setSuffix(tr(" seconds"))
        self.vu_silence_controls = QWidget()
        vu_silence_layout = QHBoxLayout(self.vu_silence_controls)
        vu_silence_layout.setContentsMargins(0, 0, 0, 0)
        vu_silence_layout.addWidget(self.vu_auto_stop)
        vu_silence_layout.addWidget(self.vu_silence_seconds)
        colors = QWidget()
    ...
    def edit_key(self, key: KeyConfig) -> None:
        ...
        widgets.extend((self.spectrum_gain, self.spectrum_fall))
        ...
        self.spectrum_gain.setValue(key.spectrum_gain_db)
        self.spectrum_fall.setValue(key.spectrum_fall_ms)
        self.spectrum_auto_stop.setChecked(key.spectrum_auto_stop)
        self.spectrum_silence_seconds.setValue(key.spectrum_silence_seconds)
    ...
    def _visual_option_changed(self) -> None:
        self._update_visibility()
        self._store()
    ...
    def _update_visibility(self) -> None:
        ...
        self.spectrum_form.setRowVisible(self.spectrum_auto_scale, spectrum_start)
        self.spectrum_form.setRowVisible(self.spectrum_gain, spectrum_start)
        self.spectrum_gain.setEnabled(not self.spectrum_auto_scale.isChecked())
        self.spectrum_form.setRowVisible(self.spectrum_fall, spectrum_start)
        self.spectrum_form.setRowVisible(self.spectrum_silence_controls, spectrum_start)
        self.spectrum_silence_seconds.setEnabled(self.spectrum_auto_stop.isChecked())
    ...
    def _store(self, *_args: object) -> None:
        ...
        self.key.spectrum_auto_scale = self.spectrum_auto_scale.isChecked()
        self.key.spectrum_gain_db = self.spectrum_gain.value()
        self.key.spectrum_fall_ms = self.spectrum_fall.value()
        self.key.spectrum_auto_stop = self.spectrum_auto_stop.isChecked()
        self.key.spectrum_silence_seconds = self.spectrum_silence_seconds.value()
